...
export const sendCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const settings = {
      ...DEFAULT_NEWSLETTER_SETTINGS,
      ...((settingsRow?.value ?? {}) as Partial<typeof DEFAULT_NEWSLETTER_SETTINGS>),
    };
    ...
    const origin = process.env["VITE_PUBLIC_SITE_URL"] || "https://liberato.com";

    type Recipient = { email: string; unsubscribe_token: string };
    let recipients: Recipient[] = [];
    if (data.testEmail) {
      recipients = [{ email: data.testEmail, unsubscribe_token: "00000000-0000-0000-0000-000000000000" }];
    } else {
    ...
    if (sent === 0) {
      return { ok: false as const, error: lastError ?? "Nenhum e-mail pôde ser enviado." };
    }
