...
function CustomerArea() {
  ...
  {current==="guia" && <><ContentTitle title={guideTool?.title || m.guide} body={guideTool?.summary || p.guideBody}/>{guideTool?<Button className="mt-6" onClick={async()=>{const result=await getDownload({data:{id:guideTool.id}});if(result.ok)window.location.assign(result.url);else toast.error(result.error)}}><Download/>{p.download}</Button>:<Button asChild className="mt-6"><Link to="/ferramentas" search={{ modo: undefined }}>{p.guideCta}</Link></Button>}</>}
  ...
  {current==="newsletter" && <><ContentTitle title={m.newsletter} body={p.newsletterBody}/><SimpleList locale={LANG_HTML[lang]} empty={p.emptyPeriod} items={(content.data?.newsletters??[]).map(item=>({id:item.id,title:item.subject,date:item.reference_date||item.published_at||item.sent_at||item.created_at,slug:item.slug}))}/></>}
  {current==="boletim" && <><ContentTitle title={m.bulletin} body={p.bulletinBody}/><SimpleList locale={LANG_HTML[lang]} empty={p.emptyPeriod} items={(content.data?.bulletins??[]).slice(0,1).map(item=>({id:item.id,title:item.subject,date:item.created_at,subtitle:item.date_label}))}/></>}
