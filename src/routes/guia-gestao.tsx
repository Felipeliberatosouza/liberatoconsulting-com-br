...
function GuidePage() {
  ...
  const p = pageText(lang).guide;
  return (
    <section className="min-h-[65vh] bg-ink py-20 text-ink-foreground">
      <div className="mx-auto max-w-5xl px-6">
        <BookOpen className="size-10 text-accent" />
        <h1 className="mt-6 max-w-4xl text-4xl font-bold leading-tight md:text-6xl">{p.title}</h1>
        <p className="mt-6 max-w-2xl text-lg text-ink-foreground/70">{p.text}</p>
        <Button asChild className="mt-8 bg-accent text-accent-foreground hover:bg-accent/90">
          <Link to="/ferramentas" search={{ modo: undefined }}>
            {p.cta} <ArrowRight />
          </Link>
        </Button>
      </div>
