    ...
    <div>
      <section className="bg-ink py-16 text-ink-foreground">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <h1 className="mt-5 text-4xl font-bold md:text-6xl">Newsletters</h1>
          <p className="mt-4 max-w-2xl text-ink-foreground/70">Publicações dos últimos três meses.</p>
          <div className="mt-8 rounded-2xl border border-accent/40 bg-ink-foreground/5 p-6">
            <p className="text-lg font-semibold">Cadastre-se para receber a newsletter</p>
            <p className="mt-1 text-sm text-ink-foreground/70">
              Receba as próximas edições por e-mail ou WhatsApp.
            </p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
          ...
          {query.data?.map((item) => (
            <article key={item.id} className="border-t-2 border-accent py-6">
              <p className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {new Date(item.reference_date || item.published_at || "").toLocaleDateString("pt-BR")}
              </p>
        ...
        {!query.isLoading && !query.data?.length && (
          <p className="text-muted-foreground">Nenhuma newsletter publicada neste período.</p>
        )}
      </section>
    </div>
  );
