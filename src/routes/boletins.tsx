    ...
    <div>
      <section className="bg-ink py-16 text-ink-foreground">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <h1 className="mt-5 text-4xl font-bold md:text-6xl">Boletins Semanais</h1>
          <p className="mt-4 max-w-2xl text-ink-foreground/70">Edições enviadas nos últimos três meses.</p>
          <a
          ...
          >
            Cadastre-se para receber o boletim semanal
          </a>
        </div>
      </section>
      <section className="mx-auto max-w-7xl px-6 py-16">
          ...
          {query.data?.map((item) => (
            <article key={item.id} className="border-t-2 border-accent py-6">
              <p className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {item.date_label || new Date(item.created_at).toLocaleDateString("pt-BR")}
              </p>
        ...
        {!query.isLoading && !query.data?.length && (
          <p className="text-muted-foreground">Nenhum boletim publicado neste período.</p>
        )}
        <div className="mt-12">
          <BulletinSignup />
        </div>
